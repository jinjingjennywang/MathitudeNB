---
title: "Experiment 1 adults' beliefs about human knowledge"
...
for(i in c(1:nrow(data_t))){
  ...
  for(j in c(1:3)){
    ...
    sum.sub$w.bias[j]<-sum.sub$When.estimate[j]-emps[j]
  }
...
summ.output.c=subset(summ.output,summ.output$SubID!=summ.output[is.na(summ.output$When.estimate),]$SubID)
t.test(w.bias~Type,paired=TRUE,data=subset(summ.output,summ.output$Type!="read"))
t.test(w.bias~Type,paired=TRUE,data=subset(summ.output.c,summ.output.c$Type!="sensory"))
...
```{r}
######### linear regression ###########
fit.lm <- lm(When.estimate~Gender+Age+Education+Parent+courses,data=output.core)
lm.beta(fit.lm)
```
...
fit.lm <- lm(How~Gender+Age+Education+Parent+courses,data=output.core)
lm.beta(fit.lm)
```
