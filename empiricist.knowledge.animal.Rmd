...
summ.output$Condition = as.factor(as.character(summ.output$Condition=="mind"))
...
descr.how <-describeBy(summ.output$How,list(summ.output$Type,summ.output$Group),na.rm=TRUE,mat=TRUE)
descr.how$confIV1 = descr.how$mean-1.96*descr.how$se
descr.how$confIV2 = descr.how$mean+1.96*descr.how$se

descr.how[c(1:3,13:15),c(2,3,6,17,18)]

```

```{r}
descr.when <-describeBy(summ.output$When,list(summ.output$Type,summ.output$Group),na.rm=TRUE,mat=TRUE)
descr.when$confIV1 = descr.when$mean-1.96*descr.when$se
descr.when$confIV2 = descr.when$mean+1.96*descr.when$se
descr.when[c(1:3,13:15),c(2,3,6,17,18)]
...
fit.lm <- lm(When~Gender+Age+Education+Parent,data=subset(output.core,output.core$Group=="animal"))
lm.beta(fit.lm)
```
...
```{r}
fit.lm <- lm(When~Gender+Age+Education+Parent,data=subset(output.core,output.core$Group=="human"))
lm.beta(fit.lm)
```
...
summ.w <- describeBy(output.core$When,list(output.core$Group,output.core$Parent),na.rm=TRUE,mat=TRUE)
summ.w$confIV1 = summ.w$mean-1.96*summ.w$se
summ.w$confIV2 = summ.w$mean+1.96*summ.w$se
summ.w[c(1,10,5,14),c(2,3,6,17,18)]

```

```{r}
fit.lm <- lm(How~Gender+Age+Education+Parent,data=subset(output.core,output.core$Group=="animal"))
lm.beta(fit.lm)
...
fit.lm <- lm(How~Gender+Age+Education+Parent,data=subset(output.core,output.core$Group=="human"))
lm.beta(fit.lm)
```
...
```{r}
summ.h <- describeBy(output.core$How,list(output.core$Group,output.core$Education),na.rm=TRUE,mat=TRUE)
summ.h$confIV1 = summ.h$mean-1.96*summ.h$se
summ.h$confIV2 = summ.h$mean+1.96*summ.h$se
summ.h[c(1,10,5,14),c(2,3,6,17,18)]
```
